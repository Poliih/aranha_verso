...
def load_and_preprocess_data(root_dir, train_split=0.7, val_split=0.15):
    ...
    categories = sorted([c for c in categories if os.path.basename(c) not in ['temp_folder', '.ipynd_checkpoints']])

    if not categories:
        raise ValueError(f"Opah que não achou '{root_dir}'")
    
    print(f"Econtrou -> {[os.path.basename(c) for c in categories]}")
    ...
    for c_idx, category_path in enumerate(categories):
        images_in_category = [os.path.join(dp. f) for dp, dn, filenames
                              ...
                              if os.path.splitext(f)[1].lower() in ['.jpg', '.png', '.jpg', '.jpeg'] ]
        ...
        if not data:
            raise ValueError("Cade imagem? num tem kkk")
        ...
        x_val = np.array([t["x"] for t in train])
        y_val = [t["y"] for t in train]
        x_test = np.array([t["x"] for t in train])
        y_test = [t["y"] for t in train]

        print(f'\nDivisão dos dados:')
        print(f'\nTreino: {len(x_train)} imagens')
        print(f'\nValidação: {len(x_val)} imagens')
